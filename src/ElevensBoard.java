...
public class ElevensBoard extends Board {
    ...
    public boolean isLegal(List<Integer> selectedCards) {
        ...
        //if there are 2 cards... return containsPairSum11
    ...
    private boolean containsPairSum11(List<Integer> selectedCards) {
        /* *** TO BE IMPLEMENTED IN ACTIVITY 9 *** */
        //OUTER LOOP
        for (int outerloop = 0; outerloop < selectedCards.size(); outerloop++) {
            // get the card at the selectedCard's indicated position
            int firstCardIndex = selectedCards.get(outerloop).intValue();
            // load the card at the given index
            Card one = this.cardAt(firstCardIndex);

             //INNER LOOP
             for (int innerloop = 0; innerloop < selectedCards.size(); innerloop++) {
                // get the card at the selectedCard's indicated position
                int secondCardIndex = selectedCards.get(innerloop).intValue();
                // load the card at the given index
                Card two = this.cardAt(secondCardIndex);
                if (firstCardIndex != secondCardIndex
                        && one.pointValue() + two.pointValue() == 11) {
                    return true;
                }
            }
        }
